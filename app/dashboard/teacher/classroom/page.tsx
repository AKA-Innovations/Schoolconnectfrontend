...
import {
  ...
  ArrowRight, Phone, Mail, RefreshCw, Search,
} from 'lucide-react';
...
export default function ClassRoomPage() {
  ...
  const attendanceMap = new Map((todayAttendance ?? []).map((a: any) => [a.studentId, a]));
  ...
  const lateCount = (todayAttendance ?? []).filter((a: any) => a.status === 'Late' || a.status === 'HalfDay').length;
