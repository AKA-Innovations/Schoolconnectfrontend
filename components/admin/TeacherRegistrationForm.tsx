  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (isEditMode) {
        ...
        // Register new teacher — include coordinatorClasses and classTeacherClass directly in the payload
        const cs = classTeacherClassId
          ? classSections.find((c: any) => c.id === classTeacherClassId)
          : null;
  ...
  return (
    <div className="max-w-5xl mx-auto px-6 py-8 animate-in fade-in duration-700">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        {/* Class Teacher Assignment — conditional on isClassTeacher */}
        <AnimatePresence>
          {formData.isClassTeacher && (
            <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }}>
              <Card className="erp-card overflow-hidden border-l-4 border-l-blue-500">
                <CardHeader className="border-b border-border/50 bg-blue-500/5 py-5 px-8">
                  <div className="flex items-center gap-3">
                    <div className="h-8 w-8 rounded-xl bg-blue-500/10 flex items-center justify-center text-blue-600">
                      <Users2 className="h-4 w-4" />
                    </div>
                    <div>
                      <CardTitle className="text-lg font-bold tracking-tight">Class Teacher Assignment</CardTitle>
                      <CardDescription className="text-xs font-medium opacity-70 mt-0.5">Select the class-section this teacher will manage as class teacher.</CardDescription>
                    </div>
                  </div>
                </CardHeader>
                <CardContent className="p-8">
